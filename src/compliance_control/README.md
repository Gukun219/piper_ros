# compliance_control — Admittance Controller 设计与测试指南

通用 ros2_control + AdmittanceController 导纳控制包，可与任意提供 URDF + MuJoCo viewer 的机械臂配合使用。默认配置针对 Piper 机械臂。
...
用户 / MoveIt
      ...
      ▼
admittance_trajectory_bridge          [ROS2 节点, compliance_control]
...
文件：`src/compliance_control/config/ros2_controllers.yaml`
...
因此位移上限设为 2.0 rad。若将测试移植到完整 MuJoCo 物理仿真，可将上限收紧至 0.3 rad。

---

### 8.5 运行测试

#### 方式一：`launch_test` CLI（推荐用于开发迭代）
...
# 轨迹跟踪精度测试
launch_test src/compliance_control/test/test_trajectory_following.py

# 导纳柔顺性测试
launch_test src/compliance_control/test/test_admittance_compliance.py
...
PIPER_VISUAL_TEST=1 launch_test \
  src/compliance_control/test/test_trajectory_following.py

PIPER_VISUAL_TEST=1 launch_test \
  src/compliance_control/test/test_admittance_compliance.py
```
...
cd ~/github_gk/piper_ros
colcon build --packages-select piper_description compliance_control piper_mujoco
colcon test --packages-select compliance_control --event-handlers console_cohesion+
colcon test-result --verbose
```

测试报告存储：`build/compliance_control/test_results/compliance_control/*.xml`
...
```bash
ros2 run compliance_control admittance_demo.py
```
...
`compliance_control` 所有机器人特定路径均通过 LaunchArgument 暴露，均有 Piper 默认值。
其他机械臂只需提供自己的 URDF xacro 和 MuJoCo viewer 节点：

```bash
ros2 launch compliance_control compliance_control.launch.py \
  xacro_file:=/path/to/myrobot.xacro \
  controllers_config:=/path/to/myrobot_controllers.yaml \
  viewer_package:=myrobot_mujoco \
  viewer_executable:=myrobot_mujoco_ctrl.py
```

| LaunchArgument | 默认值 | 说明 |
|---|---|---|
| `use_mock_hardware` | `false` | `true`=mock / `false`=MuJoCo 物理 |
| `xacro_file` | piper_description/.../piper_description_mujoco.xacro | 机器人 xacro 路径 |
| `controllers_config` | compliance_control/config/ros2_controllers.yaml | 控制器配置 yaml |
| `viewer_package` | `piper_mujoco` | MuJoCo viewer 所在包 |
| `viewer_executable` | `piper_mujoco_ctrl.py` | MuJoCo viewer 可执行文件名 |
